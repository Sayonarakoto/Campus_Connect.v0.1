...
function PrincipalLeaveReview() {
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${API}/api/staffleave/principal/pending`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setRequests(res.data.requests || []);
    } catch (err) {
      console.error("Error fetching requests:", err);
      setAlertModal({ open: true, title: "Error", message: err.response?.data?.message || "Failed to load requests", variant: "danger" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const forwardToDirector = async (id) => {
    if (!remarks[id] || remarks[id].trim() === "") {
      setAlertModal({ open: true, title: "Remarks Required", message: "Please add remarks before forwarding.", variant: "warning" });
      return;
    }
    ...
    setConfirmModal({
      ...
      message: "Are you sure you want to forward this request to the Director?",
      variant: "primary",
      confirmText: "Forward",
      onConfirm: async () => {
        ...
        try {
          ...
          await axios.put(`${API}/api/staffleave/${id}/principal-review`, { remarks: remarks[id] }, {
            headers: { Authorization: `Bearer ${token}` }
          });
          setAlertModal({ open: true, title: "Forwarded", message: "Request forwarded to Director successfully!", variant: "success" });
          setRemarks(prev => ({ ...prev, [id]: "" }));
          fetchRequests();
        } catch (err) {
          console.error("Error forwarding request:", err);
          setAlertModal({ open: true, title: "Error", message: err.response?.data?.message || "Failed to forward request", variant: "danger" });
        } finally {
              ...
              <div key={leave._id} className="admin-card">
                ...
                <div className="leave-details">
                  ...
                  <div className="detail-row">
                    ...
                    <span className={`status-badge status-${leave.status?.toLowerCase() || 'pending'}`}>{leave.status || 'Pending'}</span>
                  </div>
